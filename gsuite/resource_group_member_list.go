...
var schemaGroupMemberListEmail = map[string]*schema.Schema{
	"email": &schema.Schema{
...
var schemaGroupMemberList = mergeSchemas(schemaMember, schemaGroupMemberListEmail)

func resourceGroupMemberList() *schema.Resource {
	return &schema.Resource{
		Create: resourceGroupMemberListCreate,
		Read:   resourceGroupMemberListRead,
		Update: resourceGroupMemberListUpdate,
		Delete: resourceGroupMemberListDelete,
		Importer: &schema.ResourceImporter{
			State: resourceGroupMemberListImporter,
		},
...
func resourceGroupMemberListRead(d *schema.ResourceData, meta interface{}) error {
	log.Printf("[DEBUG]: Reading gsuite_group_member_list")
	config := meta.(*Config)

	groupEmail := d.Id()

	members, err := getAPIMemberList(groupEmail, config)
	...
	d.Set("member_list", memberListToCfg(members))
	return nil
}

func resourceGroupMemberListCreate(d *schema.ResourceData, meta interface{}) error {
	log.Printf("[DEBUG]: Creating gsuite_group_member_list")
	gid, err := createOrUpdateGroupMemberList(d, meta)
	...
	d.SetId(gid)
	return resourceGroupMemberListRead(d, meta)
}

func resourceGroupMemberListUpdate(d *schema.ResourceData, meta interface{}) error {
	log.Printf("[DEBUG]: Updating gsuite_group_member_list")
	_, err := createOrUpdateGroupMemberList(d, meta)
...
func resourceGroupMemberListDelete(d *schema.ResourceData, meta interface{}) error {
	log.Printf("[DEBUG]: Deleting gsuite_group_member_list")
	config := meta.(*Config)

	for _, rawMember := range d.Get("member_list").([]interface{}) {
		member := rawMember.(map[string]interface{})
		deleteMemberList(member["email"].(string), d.Id(), config)
	}
...
func memberListToCfg(members []*directory.Member) []map[string]interface{} {
	if members == nil {
		return nil
	}
...
func resourceMemberList(d *schema.ResourceData) (members []map[string]interface{}) {
	for _, rawMember := range d.Get("member_list").([]interface{}) {
		member := rawMember.(map[string]interface{})
...
func createOrUpdateGroupMemberList(d *schema.ResourceData, meta interface{}) (string, error) {
	config := meta.(*Config)
	groupEmail := strings.ToLower(d.Get("group_email").(string))

	// Get members from config
	cfgMembers := resourceMemberList(d)

	// Get members from API
	apiMembers, err := getAPIMemberList(groupEmail, config)
	if err != nil {
	...
	err = reconcileMemberList(d, cfgMembers, memberListToCfg(apiMembers), config, groupEmail)
	if err != nil {
		return groupEmail, fmt.Errorf("[ERROR] Error updating memberships: %v", err)
	}
...
// in the config
func reconcileMemberList(d *schema.ResourceData, cfgMembers, apiMembers []map[string]interface{}, config *Config, gid string) error {
	...
	for k, apiMember := range apiMap {
		if cfgMember, ok := cfgMap[k]; !ok {
			...
			err := deleteMemberList(k, gid, config)
			if err != nil {
				return err
			}
		} else {
	...
	for email := range cfgMap {
		err := upsertMemberList(email, gid, cfgMap[email]["role"].(string), config)
		if err != nil {
			return err
		}
	}
...
func getAPIMemberList(groupEmail string, config *Config) ([]*directory.Member, error) {
...
func upsertMemberList(email, groupEmail, role string, config *Config) error {
	var err error
	groupMember := &directory.Member{
	...
	if isGroup == false {
		...
		if err != nil {
			return createGroupMemberList(groupMember, groupEmail, config)
		}
...
func createGroupMemberList(groupMember *directory.Member, groupEmail string, config *Config) (err error) {
	var createdGroupMember *directory.Member
	err = retry(func() error {
		createdGroupMember, err = config.directory.Members.Insert(groupEmail, groupMember).Do()
		return err
...
func deleteMemberList(email, groupEmail string, config *Config) (err error) {
	err = retry(func() error {
...
func resourceGroupMemberListImporter(d *schema.ResourceData, meta interface{}) ([]*schema.ResourceData, error) {
